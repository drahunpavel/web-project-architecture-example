...
router.post("/upload", busboy(), (req, res) => {
  ...
  req.busboy.on("file", (fieldname, file, filename, mimetype) => {
    ...
    let splitParams = os.type() === "Windows_NT" ? "\\" : "/";
    let randNameArr = reqFiles.filedata.storedPFN.path.split(splitParams);
      ...
      clients.forEach((client) => {
        if (Date.now() - client.lastkeepalive > 12000) {
          ...
          logLineAsync(
            logFN,
            `[${port}] ` +
              "один из клиентов отключился, закрываем соединение с ним"
          );
        } else
          client.connection.send(
            "total:" + totalDownloaded + "/" + totalRequestLength
          ); //клиент жив
      });
    ...
    file.on("end", () => {
      fs.readFile(
        path.join(__dirname, "../files", "allFiles.json"),
        "utf8",
        (err, data) => {
          if (err) throw err;
          ...
          fs.writeFile(allFilesArr, jsonContent, "utf8", (err) => {
            ...
            logLineAsync(
              logFN,
              `[${port}] ` + `added new entry to file allFiles.json`
            );
          });
        }
      );
...
router.get("/:id/download", (req, res) => {
  ...
  fs.readFile(
    ...
    (err, data) => {
      ...
      let parsData = JSON.parse(data);
      let actualFile = parsData.filter((item) => {
        return item.tmp_filename === id;
      });
      ...
      logLineAsync(
        logFN,
        `[${port}] ` + `file ${actualFile[0].filename} uploaded`
      );
